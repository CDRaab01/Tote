...
async def _owned(db: AsyncSession, model, user_id: uuid.UUID, obj_id: uuid.UUID):
    obj = (
        await db.execute(select(model).where(model.id == obj_id, model.user_id == user_id))
    ).scalar_one_or_none()
    if obj is None:
...
async def create_location(body: LocationIn, user: CurrentUser, db: Db):
    if body.parent_id:
        await _owned(db, Location, user.id, body.parent_id)
...
async def patch_location(location_id: uuid.UUID, body: LocationPatch, user: CurrentUser, db: Db):
    loc = await _owned(db, Location, user.id, location_id)
    ...
    if updates.get("parent_id"):
        await _owned(db, Location, user.id, updates["parent_id"])
...
async def patch_category(category_id: uuid.UUID, body: CategoryPatch, user: CurrentUser, db: Db):
    cat = await _owned(db, Category, user.id, category_id)
